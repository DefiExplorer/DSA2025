...
{   
    ...
    public static void removeDup(int[] arr)
    {
        ...
        while(true)
        {
            ...
            if(arr[j] != arr[i] && j<arr.length)
            {
                arr[++i] = arr[j];
                unique = arr[i];
                arr[j]=0; // replace position with 0, once it is copied.
            }
            else{arr[j]=0;}// replace other duplicate with 0
            j++;
	...
	{   
	    ...
	    System.out.println();
	    for(int j=0;j<n;j++)
	    {   if(arr[j]==0){break;} // All Unique elements are printed, so ignore rest.
	        System.out.print(arr[j]+" ");//Prints the sorted array.
	    }
	}
}
